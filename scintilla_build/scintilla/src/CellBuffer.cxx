...
struct CountWidths {
  ...
  CountWidths(Sci::Position countBasePlane_ = 0,
              Sci::Position countOtherPlanes_ = 0) noexcept
      : countBasePlane(countBasePlane_), countOtherPlanes(countOtherPlanes_) {}
...
public:
  ...
  virtual void InsertLine(Sci::Line line, Sci::Position position,
                          bool lineStart) = 0;
  virtual void SetLineStart(Sci::Line line,
                            Sci::Position position) noexcept = 0;
  ...
  virtual bool AllocateLineCharacterIndex(int lineCharacterIndex,
                                          Sci::Line lines) = 0;
  virtual bool ReleaseLineCharacterIndex(int lineCharacterIndex) = 0;
  virtual Sci::Position
  IndexLineStart(Sci::Line line, int lineCharacterIndex) const noexcept = 0;
  virtual Sci::Line
  LineFromPositionIndex(Sci::Position pos,
                        int lineCharacterIndex) const noexcept = 0;
  virtual ~ILineVector() {}
};
...
public:
  ...
  virtual ~LineStartIndex() { starts.DeleteAll(); }
  bool Allocate(Sci::Line lines) {
    ...
    for (Sci::Line line = starts.Partitions(); line < lines; line++) {
      // Produce an ascending sequence that will be filled in with correct
      // widths later
      length++;
  ...
  bool Active() const noexcept { return refCount > 0; }
  ...
  void SetLineWidth(Sci::Line line, Sci::Position width) {
    ...
    starts.InsertText(static_cast<POS>(line),
                      static_cast<POS>(width - widthCurrent));
  }
};
...
public:
  LineVector() : starts(256), perLine(0) { Init(); }
  ...
  void SetPerLine(PerLine *pl) override { perLine = pl; }
  void InsertText(Sci::Line line, Sci::Position delta) override {
    starts.InsertText(static_cast<POS>(line), static_cast<POS>(delta));
  }
  void InsertLine(Sci::Line line, Sci::Position position,
                  bool lineStart) override {
    const POS lineAsPos = static_cast<POS>(line);
    starts.InsertPartition(lineAsPos, static_cast<POS>(position));
    if (startsUTF32.Active()) {
      startsUTF32.starts.InsertPartition(
          lineAsPos,
          static_cast<POS>(
              startsUTF32.starts.PositionFromPartition(lineAsPos - 1) + 1));
    }
    if (startsUTF16.Active()) {
      startsUTF16.starts.InsertPartition(
          lineAsPos,
          static_cast<POS>(
              startsUTF16.starts.PositionFromPartition(lineAsPos - 1) + 1));
    }
    if (perLine) {
      if ((line > 0) && lineStart)
        line--;
  ...
  void SetLineStart(Sci::Line line, Sci::Position position) noexcept override {
    starts.SetPartitionStartPosition(static_cast<POS>(line),
                                     static_cast<POS>(position));
  }
  ...
  Sci::Line LineFromPosition(Sci::Position pos) const noexcept override {
    return static_cast<Sci::Line>(
        starts.PartitionFromPosition(static_cast<POS>(pos)));
  }
  ...
  void InsertCharacters(Sci::Line line, CountWidths delta) override {
    if (startsUTF32.Active()) {
      startsUTF32.starts.InsertText(static_cast<POS>(line),
                                    static_cast<POS>(delta.WidthUTF32()));
    }
    if (startsUTF16.Active()) {
      startsUTF16.starts.InsertText(static_cast<POS>(line),
                                    static_cast<POS>(delta.WidthUTF16()));
    }
  }
  ...
  bool AllocateLineCharacterIndex(int lineCharacterIndex,
                                  Sci::Line lines) override {
    bool changed = false;
  ...
  Sci::Position IndexLineStart(Sci::Line line,
                               int lineCharacterIndex) const noexcept override {
  ...
  LineFromPositionIndex(Sci::Position pos,
    ...
    if (lineCharacterIndex == SC_LINECHARACTERINDEX_UTF32) {
      return static_cast<Sci::Line>(
          startsUTF32.starts.PartitionFromPosition(static_cast<POS>(pos)));
    } else {
      return static_cast<Sci::Line>(
          startsUTF16.starts.PartitionFromPosition(static_cast<POS>(pos)));
    }
  }
};
...
void Action::Create(actionType at_, Sci::Position position_, const char *data_,
                    Sci::Position lenData_, bool mayCoalesce_) {
  data = nullptr;
  position = position_;
  at = at_;
  if (lenData_) {
...
// The undo history stores a sequence of user operations that represent the
// user's view of the commands executed on the text. Each user operation
// contains a sequence of text insertion and text deletion actions. All the user
// operations are stored in a list of individual actions with 'start' actions
// used as delimiters between user operations. Initially there is one start
// action in the history. As each action is performed, it is recorded in the
// history. The action may either become part of the current user operation or
// may start a new user operation. If it is to be part of the current operation,
// then it overwrites the current last action. If it is to be part of a new
// operation, it is appended after the current last action.
// After writing the new action, a new start action is appended at the end of
// the history. The decision of whether to start a new user operation is based
// upon two factors. If a compound operation has been explicitly started by
// calling BeginUndoAction and no matching EndUndoAction (these calls nest) has
// been called, then the action is coalesced into the current operation. If
// there is no outstanding BeginUndoAction call then a new operation is started
// unless it looks as if the new action is caused by the user typing or deleting
// a stream of text. Sequences that look like typing or deletion are coalesced
...
const char *UndoHistory::AppendAction(actionType at, Sci::Position position,
                                      const char *data,
                                      Sci::Position lengthData,
                                      bool &startSequence, bool mayCoalesce) {
  EnsureUndoRoom();
  // Platform::DebugPrintf("%% %d action %d %d %d\n", at, position, lengthData,
  // currentAction); Platform::DebugPrintf("^ %d action %d %d\n",
  // actions[currentAction - 1].at, 	actions[currentAction - 1].position,
  //actions[currentAction - 1].lenData);
    ...
    if (0 == undoSequenceDepth) {
      ...
      if ((currentAction == savePoint) || (currentAction == tentativePoint)) {
        ...
        currentAction++;
                 ...
                 actions[currentAction].at == containerAction) {
      ...
      // Actions not at top level are always coalesced unless this is after
      // return to top level
...
void UndoHistory::DropUndoSequence() { undoSequenceDepth = 0; }
...
void UndoHistory::SetSavePoint() { savePoint = currentAction; }

bool UndoHistory::IsSavePoint() const { return savePoint == currentAction; }

void UndoHistory::TentativeStart() { tentativePoint = currentAction; }
...
void UndoHistory::CompletedUndoStep() { currentAction--; }

bool UndoHistory::CanRedo() const { return maxAction > currentAction; }
...
void UndoHistory::CompletedRedoStep() { currentAction++; }
...
void CellBuffer::GetCharRange(char *buffer, Sci::Position position,
...
void CellBuffer::GetStyleRange(unsigned char *buffer, Sci::Position position,
                               Sci::Position lengthRetrieve) const {
  if (lengthRetrieve < 0)
    return;
  if (position < 0)
...
const char *CellBuffer::BufferPointer() { return substance.BufferPointer(); }
...
const char *CellBuffer::InsertString(Sci::Position position, const char *s,
                                     Sci::Position insertLength,
                                     bool &startSequence) {
  // InsertString and DeleteChars are the bottleneck though which all changes
  // occur
  const char *data = s;
  if (!readOnly) {
    if (collectingUndo) {
      // Save into the undo/redo stack, but only the characters - not the
      // formatting This takes up about half load time
      data = uh.AppendAction(insertAction, position, s, insertLength,
                             startSequence);
    }
...
bool CellBuffer::SetStyleFor(Sci::Position position, Sci::Position lengthStyle,
                             char styleValue) {
  if (!hasStyles) {
    return false;
  }
...
const char *CellBuffer::DeleteChars(Sci::Position position,
  ...
  if (!readOnly) {
    if (collectingUndo) {
      // Save into the undo/redo stack, but only the characters - not the
      // formatting The gap would be moved to position anyway for the deletion
      // so this doesn't cost extra
      data = substance.RangePointer(position, deleteLength);
      data = uh.AppendAction(removeAction, position, data, deleteLength,
                             startSequence);
    }
...
Sci::Position CellBuffer::Length() const noexcept { return substance.Length(); }
...
void CellBuffer::SetPerLine(PerLine *pl) { plv->SetPerLine(pl); }
...
Sci::Line CellBuffer::Lines() const noexcept { return plv->Lines(); }
...
Sci::Position
CellBuffer::IndexLineStart(Sci::Line line,
                           int lineCharacterIndex) const noexcept {
  return plv->IndexLineStart(line, lineCharacterIndex);
}

Sci::Line
CellBuffer::LineFromPositionIndex(Sci::Position pos,
                                  int lineCharacterIndex) const noexcept {
  return plv->LineFromPositionIndex(pos, lineCharacterIndex);
}

bool CellBuffer::IsReadOnly() const { return readOnly; }

void CellBuffer::SetReadOnly(bool set) { readOnly = set; }

bool CellBuffer::IsLarge() const { return largeDocument; }

bool CellBuffer::HasStyles() const { return hasStyles; }

void CellBuffer::SetSavePoint() { uh.SetSavePoint(); }

bool CellBuffer::IsSavePoint() const { return uh.IsSavePoint(); }

void CellBuffer::TentativeStart() { uh.TentativeStart(); }

void CellBuffer::TentativeCommit() { uh.TentativeCommit(); }

int CellBuffer::TentativeSteps() { return uh.TentativeSteps(); }

bool CellBuffer::TentativeActive() const { return uh.TentativeActive(); }

// Without undo

void CellBuffer::InsertLine(Sci::Line line, Sci::Position position,
                            bool lineStart) {
  plv->InsertLine(line, position, lineStart);
}

void CellBuffer::RemoveLine(Sci::Line line) { plv->RemoveLine(line); }
...
bool CellBuffer::UTF8LineEndOverlaps(Sci::Position position) const {
  ...
  return UTF8IsSeparator(bytes) || UTF8IsSeparator(bytes + 1) ||
         UTF8IsNEL(bytes + 1);
}
...
void CellBuffer::RecalculateIndexLineStarts(Sci::Line lineFirst,
  ...
  for (Sci::Line line = lineFirst; line <= lineLast; line++) {
    // Find line start and end, retrieve text of line, count characters and
    // update line width
...
void CellBuffer::BasicInsertString(Sci::Position position, const char *s,
                                   Sci::Position insertLength) {
  if (insertLength == 0)
    return;
  ...
  // A simple insertion is one that inserts valid text on a single line at a
  // character boundary
  bool simpleInsertion = false;

  // Check for breaking apart a UTF-8 sequence and inserting invalid UTF-8
  if (utf8Substance &&
      (plv->LineCharacterIndex() != SC_LINECHARACTERINDEX_NONE)) {
    // Actually, don't need to check that whole insertion is valid just that
    // there are no potential fragments at ends.
  ...
  // Joining two lines where last insertion is cr and following substance starts
  // with lf
  if (chAfter == '\n') {
    if (ch == '\r') {
  ...
  if (simpleInsertion) {
    const CountWidths cw =
        CountCharacterWidthsUTF8(std::string_view(s, insertLength));
...
void CellBuffer::BasicDeleteChars(Sci::Position position,
                                  Sci::Position deleteLength) {
  if (deleteLength == 0)
    return;
  ...
  if ((position == 0) && (deleteLength == substance.Length())) {
    ...
    // Have to fix up line positions before doing deletion as looking at text in
    // buffer to work out which lines have been removed
    ...
    // Needs further checks that text is UTF-8 or that some other break apart is
    // occurring
    if (utf8Substance &&
        (plv->LineCharacterIndex() != SC_LINECHARACTERINDEX_NONE)) {
      const Sci::Position posEnd = position + deleteLength;
      const Sci::Line lineEndRemove = plv->LineFromPosition(posEnd);
      const bool simpleDeletion = (linePosition == lineEndRemove) &&
                                  UTF8IsCharacterBoundary(position) &&
                                  UTF8IsCharacterBoundary(posEnd);
      if (simpleDeletion) {
...
bool CellBuffer::IsCollectingUndo() const { return collectingUndo; }

void CellBuffer::BeginUndoAction() { uh.BeginUndoAction(); }

void CellBuffer::EndUndoAction() { uh.EndUndoAction(); }
...
void CellBuffer::DeleteUndoHistory() { uh.DeleteUndoHistory(); }

bool CellBuffer::CanUndo() const { return uh.CanUndo(); }

int CellBuffer::StartUndo() { return uh.StartUndo(); }

const Action &CellBuffer::GetUndoStep() const { return uh.GetUndoStep(); }

void CellBuffer::PerformUndoStep() {
  const Action &actionStep = uh.GetUndoStep();
  if (actionStep.at == insertAction) {
    if (substance.Length() < actionStep.lenData) {
      throw std::runtime_error("CellBuffer::PerformUndoStep: deletion must be "
                               "less than document length.");
    }
    BasicDeleteChars(actionStep.position, actionStep.lenData);
  } else if (actionStep.at == removeAction) {
    BasicInsertString(actionStep.position, actionStep.data.get(),
                      actionStep.lenData);
  }
  uh.CompletedUndoStep();
}

bool CellBuffer::CanRedo() const { return uh.CanRedo(); }

int CellBuffer::StartRedo() { return uh.StartRedo(); }

const Action &CellBuffer::GetRedoStep() const { return uh.GetRedoStep(); }
