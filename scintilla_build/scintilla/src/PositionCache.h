...
public:
  ...
  explicit PointDocument(double x_ = 0, double y_ = 0) noexcept
      : x(x_), y(y_) {}
...
public:
  ...
  void SetBracesHighlight(Range rangeLine, const Sci::Position braces[],
                          ...
                          bool ignoreStyle);
  void RestoreBracesHighlight(Range rangeLine, const Sci::Position braces[],
                              bool ignoreStyle);
...
struct ScreenLine : public IScreenLine {
  ...
  ScreenLine(const LineLayout *ll_, int subLine, const ViewStyle &vs,
             XYPOSITION width_, int tabWidthMinimumPixels_);
...
public:
  ...
  LineLayout *Retrieve(Sci::Line lineNumber, Sci::Line lineCaret, int maxChars,
                       int styleClock_, Sci::Line linesOnScreen,
                       Sci::Line linesInDoc);
...
public:
  PositionCacheEntry();
  // Copy constructor not currently used, but needed for being element in
  // std::vector.
  ...
  PositionCacheEntry &operator=(PositionCacheEntry &&) noexcept = default;
  ~PositionCacheEntry();
  void Set(unsigned int styleNumber_, const char *s_, unsigned int len_,
           XYPOSITION *positions_, unsigned int clock_);
  void Clear();
  bool Retrieve(unsigned int styleNumber_, const char *s_, unsigned int len_,
                XYPOSITION *positions_) const;
  static unsigned int Hash(unsigned int styleNumber_, const char *s,
                           unsigned int len_);
...
public:
  ...
  const Representation *RepresentationFromCharacter(const char *charBytes,
                                                    size_t len) const;
...
struct TextSegment {
  ...
  TextSegment(int start_ = 0, int length_ = 0,
              const Representation *representation_ = nullptr) noexcept
...
public:
  ...
  BreakFinder(const LineLayout *ll_, const Selection *psel, Range lineRange_,
              Sci::Position posLineStart_, int xStart, bool breakForSelection,
              const Document *pdoc_, const SpecialRepresentations *preprs_,
...
public:
  ...
  void MeasureWidths(Surface *surface, const ViewStyle &vstyle,
                     unsigned int styleNumber, const char *s, unsigned int len,
                     XYPOSITION *positions, const Document *pdoc);
};
