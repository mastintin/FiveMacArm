...
%CC% -c -O2 -tW -I%HDIR%\include -I%CDIR%\include %PROG%.c > cc.log 2>&1
if errorlevel 1 (
   type cc.log
...
%ILINK% -Gn -aa -Tpe -L%CDIR%\lib -L%CDIR%\lib\psdk -L%HDIR%\lib\win\bcc ^
   ...
   hbdebug.lib gtwin.lib gtwvt.lib gtwvg.lib gtgui.lib ^
...
echo.
echo * %PROG%.exe built successfully *
start %PROG%
