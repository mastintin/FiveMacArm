...
static const HB_GC_FUNCS s_gcXLSXWorksheetFuncs = {XLSXWorksheet_release,
                                                   hb_worksheet_mark};

void hb_XLSXWorksheet_ret(lxw_worksheet *p) {
  // fprintf( stderr,"Chiamato hb_XLSXWorksheet_ret\n" );
  if (p) {
    PHB_WORKSHEET_GC pGC = (PHB_WORKSHEET_GC)hb_gcAllocate(
        sizeof(HB_WORKSHEET_GC), &s_gcXLSXWorksheetFuncs);
    pGC->worksheet = p;
...
lxw_worksheet *hb_XLSXWorksheet_par(int iParam) {
  ...
  PHB_WORKSHEET_GC pGC =
      (PHB_WORKSHEET_GC)hb_parptrGC(&s_gcXLSXWorksheetFuncs, iParam);
...
lxw_worksheet *hb_XLSXWorksheet_item(PHB_ITEM pValue) {
  PHB_WORKSHEET_GC pGC =
      (PHB_WORKSHEET_GC)hb_itemGetPtrGC(pValue, &s_gcXLSXWorksheetFuncs);
...
lxw_image_options *image_options(PHB_ITEM pHash) {
  if (HB_IS_HASH(pHash)) {
    lxw_image_options *image_data =
        (lxw_image_options *)hb_xalloc(sizeof(lxw_image_options));
...
static lxw_comment_options *comment_options_from_hash(PHB_ITEM pHash) {
  if (HB_IS_HASH(pHash)) {
    lxw_comment_options *options =
        (lxw_comment_options *)hb_xalloc(sizeof(lxw_comment_options));
    memset(options, 0, sizeof(lxw_comment_options));
    ...
    while (++nPos <= nLen) {
      ...
      PHB_ITEM pValue = hb_hashGetValueAt(pHash, nPos);
      if (pKey && pValue) {
        const char *key = hb_itemGetCPtr(pKey);

        if (HB_IS_STRING(pValue)) {
          const char *value = hb_itemGetCPtr(pValue);
          if (hb_stricmp(key, "author") == 0)
            options->author = value;
          else if (hb_stricmp(key, "font_name") == 0)
            options->font_name = value;
        } else if (HB_IS_NUMERIC(pValue)) {
          double value = hb_itemGetND(pValue);
          if (hb_stricmp(key, "width") == 0)
            options->width = (uint16_t)value;
          else if (hb_stricmp(key, "height") == 0)
            options->height = (uint16_t)value;
          else if (hb_stricmp(key, "x_scale") == 0)
            options->x_scale = value;
          else if (hb_stricmp(key, "y_scale") == 0)
            options->y_scale = value;
          else if (hb_stricmp(key, "color") == 0)
            options->color = (lxw_color_t)value;
          else if (hb_stricmp(key, "font_size") == 0)
            options->font_size = value;
          else if (hb_stricmp(key, "visible") == 0)
            options->visible = (uint8_t)value;
          else if (hb_stricmp(key, "start_row") == 0)
            options->start_row = (lxw_row_t)value;
          else if (hb_stricmp(key, "start_col") == 0)
            options->start_col = (lxw_col_t)value;
          else if (hb_stricmp(key, "x_offset") == 0)
            options->x_offset = (int32_t)value;
          else if (hb_stricmp(key, "y_offset") == 0)
            options->y_offset = (int32_t)value;
        }
      }
    }
    return options;
...
//   lxw_image_options *image_data = hb_parptr(4 ) ;
   ...
   lxw_worksheet_prepare_chart( self, chart_ref_id, drawing_id, image_data,
is_chartsheet) ; hb_xfree( image_data );
}
*/
...
HB_FUNC(WORKSHEET_WRITE_FORMULA_NUM) {
  ...
  if (self)
    hb_retni(worksheet_write_formula_num(self, row_num, col_num, formula,
                                         format, result));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_WRITE_ARRAY_FORMULA_NUM) {
  ...
  if (self)
    hb_retni(worksheet_write_array_formula_num(self, first_row, first_col,
...
HB_FUNC(WORKSHEET_WRITE_ARRAY_FORMULA) {
  ...
  if (self)
    hb_retni(worksheet_write_array_formula(self, first_row, first_col, last_row,
                                           last_col, formula, format));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_WRITE_DATETIME) {
  ...
  if (self)
    hb_retni(
        worksheet_write_datetime(self, row_num, col_num, &datetime, format));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_WRITE_URL_OPT) {
  ...
  if (self)
    hb_retni(worksheet_write_url_opt(self, row_num, col_num, url, format,
                                     string, tooltip));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_WRITE_RICH_STRING) {
  ...
  if (pArray && HB_IS_ARRAY(pArray) && hb_arrayLen(pArray) > 0 && self) {
    ...
    lxw_rich_string_tuple **rich_strings = (lxw_rich_string_tuple **)hb_xalloc(
        sizeof(lxw_rich_string_tuple) * (nLen + 1));
    ...
    while (++nPos <= nLen) {
      ...
      lxw_rich_string_tuple *tuple =
          (lxw_rich_string_tuple *)hb_xalloc(sizeof(lxw_rich_string_tuple));
      ...
      if (pHash && hb_hashLen(pHash) > 0) {
        ...
        hb_errRT_BASE(EG_ARG, 0, "Not LXW_RICH_STRING Hash ", HB_ERR_FUNCNAME,
    ...
    if (nLen > 0) {
      ...
      hb_retni(worksheet_write_rich_string(self, row_num, col_num, rich_strings,
                                           format));
...
lxw_row_col_options *hash2row_col_options(PHB_ITEM pHash) {
  if (HB_IS_HASH(pHash)) {
    lxw_row_col_options *properties =
        (lxw_row_col_options *)hb_xalloc(sizeof(lxw_row_col_options));
...
HB_FUNC(WORKSHEET_SET_COLUMN_OPT) {
  ...
  if (self) {
    ...
    hb_retni(worksheet_set_column_opt(self, firstcol, lastcol, width, format,
                                      user_options));
  } else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_SET_COLUMN_PIXELS) {
  ...
  if (self)
    worksheet_set_column_pixels(self, first_col, last_col, width, format);
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_SET_ROW_OPT) {
  ...
  if (self)
    hb_retni(
        worksheet_set_row_opt(self, row_num, height, format, user_options));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_SET_ROW_PIXELS) {
  ...
  if (self)
    worksheet_set_row_pixels(self, row, height, format);
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_MERGE_RANGE) {
  ...
  if (self)
    hb_retni(worksheet_merge_range(self, first_row, first_col, last_row,
                                   last_col, string, format));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_AUTOFILTER) {
  ...
  if (self)
    hb_retni(
        worksheet_autofilter(self, first_row, first_col, last_row, last_col));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_FREEZE_PANES_OPT) {
  ...
  if (self)
    worksheet_freeze_panes_opt(self, first_row, first_col, top_row, left_col,
                               type);
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_PRINT_AREA) {
  ...
  if (self)
    hb_retni(
        worksheet_print_area(self, first_row, first_col, last_row, last_col));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_OUTLINE_SETTINGS) {
  ...
  if (self)
    worksheet_outline_settings(self, visible, symbols_below, symbols_right,
                               auto_style);
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_INSERT_IMAGE_OPT) {
  ...
  if (self) {
    ...
    hb_retni(worksheet_insert_image_opt(self, row_num, col_num, filename,
                                        image_data));
    hb_xfree(image_data);
  } else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
  /*
     ...
     lxw_image_options *image_data = (lxw_image_options *) hb_xalloc(
     sizeof(lxw_image_options) );
     ...
     if( pHash )
     {
        ...
        if( image_data ){
           hb_retni( worksheet_insert_image_opt( self, row_num, col_num,
     filename, image_data ) ); hb_xfree( image_data );
        }
     }
  */
}
...
HB_FUNC(WORKSHEET_INSERT_IMAGE_BUFFER) {
  ...
  lxw_row_t row_num = hb_parni(2);
  lxw_col_t col_num = hb_parni(3);
  const unsigned char *image_buffer = (const unsigned char *)hb_parcx(4);
  HB_SIZE nSize = hb_parclen(4);

  if (self && image_buffer)
    hb_retni(worksheet_insert_image_buffer(self, row_num, col_num, image_buffer,
                                           (size_t)nSize));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_WRITE_COMMENT) {
  ...
  const char *comment = hb_parcx(4);

  if (self)
    hb_retni(worksheet_write_comment(self, row, col, comment));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
HB_FUNC(WORKSHEET_WRITE_COMMENT_OPT) {
  ...
  if (self) {
    lxw_comment_options *options = comment_options_from_hash(pHash);
    hb_retni(worksheet_write_comment_opt(self, row, col, comment, options));
    if (options)
      hb_xfree((void *)options);
  } else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
...
//   lxw_image_options *user_options = hb_parptr( 5 ) ;
   ...
   if( pHash ){
      //lxw_image_options *user_options ;
      //lxw_image_options *user_options = (lxw_image_options *)
image_options( pHash ) ; lxw_image_options *user_options = image_options(
pHash ) ;

      hb_retni( worksheet_insert_chart_opt( self, row_num, col_num, chart,
user_options ) ); hb_xfree( user_options );
   }
}
*/
...
HB_FUNC(WORKSHEET_DATA_VALIDATION_RANGE) {
  ...
  if (self)
    hb_retni(worksheet_data_validation_range(self, first_row, first_col,
                                             last_row, last_col, validation));
  else
    hb_errRT_BASE(EG_ARG, 2020, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
}
 ...
 *                                lxw_col_t col, lxw_data_validation
 * *validation)
 *
 */
/*
...
HB_FUNC(WORKSHEET_DATA_VALIDATION_CELL) {
  ...
  lxw_data_validation *validation =
      (lxw_data_validation *)hb_xalloc(sizeof(lxw_data_validation));
  if (validation == NULL) {
    hb_errRT_BASE(EG_MEM, 0, NULL, HB_ERR_FUNCNAME, HB_ERR_ARGS_BASEPARAMS);
  }
  ...
  if (pHash) {
        ...
        if (HB_IS_NUMERIC(pValue)) {
          ...
          if (nLen) {
            validation->value_list =
                (const char **)hb_xalloc(sizeof(char *) * (nLen + 1));
            if (validation->value_list == NULL) {
              hb_errRT_BASE(EG_MEM, 0, NULL, HB_ERR_FUNCNAME,
                            HB_ERR_ARGS_BASEPARAMS);
            } else {
              memset((void *)validation->value_list, 0,
                     sizeof(char *) * (nLen + 1));
              HB_SIZE nIndex = 0;
              while (nIndex < nLen) {
                validation->value_list[nIndex] =
                    (const char *)hb_arrayGetC(pValue, nIndex + 1);
                nIndex++;
              }
            }
          }
    ...
    if (validation) {
      hb_retni(worksheet_data_validation_range(self, row, col, row, col,
                                               validation));
      HB_SIZE nIndex = 0;
      if (validation->value_list) {
        while (validation->value_list[nIndex]) {
          hb_xfree((void *)validation->value_list[nIndex]);
          nIndex++;
        }
        hb_xfree((void *)validation->value_list);
      }
      hb_xfree((void *)validation);
    }
  }
}
...
HB_FUNC(WORKSHEET_ADD_TABLE) {
  ...
  if (self) {
    lxw_table_options *options = NULL;
    if (pHash) {
      options = (lxw_table_options *)hb_xalloc(sizeof(lxw_table_options));
      memset(options, 0, sizeof(lxw_table_options));
      ...
      while (++nPos <= nLen) {
        ...
        if (pKey && pValue) {
          const char *key = hb_itemGetCPtr(pKey);
          if (hb_stricmp(key, "name") == 0)
            options->name = hb_itemGetCPtr(pValue);
          else if (hb_stricmp(key, "no_header_row") == 0)
            options->no_header_row = (uint8_t)hb_itemGetL(pValue);
          else if (hb_stricmp(key, "no_autofilter") == 0)
            options->no_autofilter = (uint8_t)hb_itemGetL(pValue);
          else if (hb_stricmp(key, "no_banded_rows") == 0)
            options->no_banded_rows = (uint8_t)hb_itemGetL(pValue);
          else if (hb_stricmp(key, "banded_columns") == 0)
            options->banded_columns = (uint8_t)hb_itemGetL(pValue);
          else if (hb_stricmp(key, "first_column") == 0)
            options->first_column = (uint8_t)hb_itemGetL(pValue);
          else if (hb_stricmp(key, "last_column") == 0)
            options->last_column = (uint8_t)hb_itemGetL(pValue);
          else if (hb_stricmp(key, "style_type") == 0)
            options->style_type = (uint8_t)hb_itemGetNI(pValue);
          else if (hb_stricmp(key, "style_type_number") == 0)
            options->style_type_number = (uint8_t)hb_itemGetNI(pValue);
          else if (hb_stricmp(key, "total_row") == 0)
            options->total_row = (uint8_t)hb_itemGetL(pValue);
          else if (hb_stricmp(key, "columns") == 0 && HB_IS_ARRAY(pValue)) {
            HB_SIZE nCols = hb_arrayLen(pValue);
            if (nCols > 0) {
              options->columns = (lxw_table_column **)hb_xalloc(
                  sizeof(lxw_table_column *) * (nCols + 1));
              memset(options->columns, 0,
                     sizeof(lxw_table_column *) * (nCols + 1));
              for (HB_SIZE i = 0; i < nCols; i++) {
                PHB_ITEM pColHash = hb_arrayGetItemPtr(pValue, i + 1);
                if (HB_IS_HASH(pColHash)) {
                  options->columns[i] =
                      (lxw_table_column *)hb_xalloc(sizeof(lxw_table_column));
                  memset(options->columns[i], 0, sizeof(lxw_table_column));
                  HB_SIZE nCLen = hb_hashLen(pColHash), nCPos = 0;
                  while (++nCPos <= nCLen) {
                    PHB_ITEM pCKey = hb_hashGetKeyAt(pColHash, nCPos);
                    PHB_ITEM pCValue = hb_hashGetValueAt(pColHash, nCPos);
                    if (pCKey && pCValue) {
                      const char *ckey = hb_itemGetCPtr(pCKey);
                      if (hb_stricmp(ckey, "header") == 0)
                        options->columns[i]->header = hb_itemGetCPtr(pCValue);
                      else if (hb_stricmp(ckey, "formula") == 0)
                        options->columns[i]->formula = hb_itemGetCPtr(pCValue);
                      else if (hb_stricmp(ckey, "total_string") == 0)
                        options->columns[i]->total_string =
                            hb_itemGetCPtr(pCValue);
                      else if (hb_stricmp(ckey, "total_function") == 0)
                        options->columns[i]->total_function =
                            (uint8_t)hb_itemGetNI(pCValue);
                      else if (hb_stricmp(ckey, "total_value") == 0)
                        options->columns[i]->total_value =
                            hb_itemGetND(pCValue);
                      else if (hb_stricmp(ckey, "format") == 0)
                        options->columns[i]->format =
                            hb_XLSXFormat_item(pCValue);
                      else if (hb_stricmp(ckey, "header_format") == 0)
                        options->columns[i]->header_format =
                            hb_XLSXFormat_item(pCValue);
                    }
                  }
                }
    ...
    hb_retni(worksheet_add_table(self, first_row, first_col, last_row, last_col,
                                 options));
    if (options) {
      if (options->columns) {
        for (HB_SIZE i = 0; options->columns[i]; i++) {
          hb_xfree((void *)options->columns[i]);
        }
        hb_xfree((void *)options->columns);
      }
      hb_xfree((void *)options);
    }
  } else
