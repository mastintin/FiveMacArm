...
#define LXW_MAX_FORMULA_RANGE_LENGTH                                           \
  (LXW_MAX_SHEETNAME_LENGTH + LXW_MAX_CELL_RANGE_LENGTH)
...
#define LXW_WARN(message) LXW_PRINTF(LXW_STDERR "[WARNING]: " message "\n")
...
#define LXW_WARN_CAT_AXIS_ONLY(function)                                       \
  do {                                                                         \
    if (!axis->is_category) {                                                  \
      LXW_PRINTF(LXW_STDERR "[WARNING]: " function                             \
                            "() is only valid for category axes\n");           \
      return;                                                                  \
    }                                                                          \
  } while (0)
...
#define LXW_WARN_CAT_AND_DATE_AXIS_ONLY(function)                              \
  do {                                                                         \
    if (!axis->is_category && !axis->is_date) {                                \
      LXW_PRINTF(LXW_STDERR "[WARNING]: " function                             \
                            "() is only valid for category and date axes\n");  \
      return;                                                                  \
    }                                                                          \
  } while (0)
...
#define LXW_WARN_VALUE_AND_DATE_AXIS_ONLY(function)                            \
  do {                                                                         \
    if (!axis->is_value && !axis->is_date) {                                   \
      LXW_PRINTF(LXW_STDERR "[WARNING]: " function                             \
                            "() is only valid for value and date axes\n");     \
      return;                                                                  \
    }                                                                          \
  } while (0)
...
__inline int msvc2010_vsnprintf(char *str, size_t size, const char *format,
                                va_list ap) {
  int count = -1;
...
__inline int msvc2010_snprintf(char *str, size_t size, const char *format,
                               ...) {
  int count;
  va_list ap;
