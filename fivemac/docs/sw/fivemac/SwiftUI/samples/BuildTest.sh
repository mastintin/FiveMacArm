...
clang -target arm64-apple-macosx26.0 -c "obj/$APP_NAME.c" \
    -I"../include" \
...
swiftc -target arm64-apple-macosx26.0 -o "$APP_NAME.app/Contents/MacOS/$APP_NAME" \
    "obj/$APP_NAME.o" \
...
echo '   <key>NSSpeechRecognitionUsageDescription</key>' >> "$PLIST"
echo '   <string>Permission is required to record your speech and convert it into text.</string>' >> "$PLIST"
echo '   <key>NSMicrophoneUsageDescription</key>' >> "$PLIST"
echo '   <string>Permission is required to access the microphone for speech recognition.</string>' >> "$PLIST"
echo '</dict>' >> "$PLIST"
echo '</plist>' >> "$PLIST"
