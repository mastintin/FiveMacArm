...
Function Imprime()
local oPrn:=TPrinter():new(0,0,1000,1000)
...
cFoto :=  cGetfile("escoge la imagen ")
   ...
   nH := oPrn:GetPrintableHeight()
   nW := oPrn:GetPrintableWidth()
   
   VIEWSETSIZE(oprn:hWnd,nW,nH )
   
   
 //   oprn:SetRightMargin(0)
 
 //   @ 0, 0 SAY "A FiveMac Say " OF oPrn SIZE 150, 20 RAISED
...
// oprn:SetRightMargin(0)
  ...
  // nH := oPrn:GetPrintableHeight()
   //nW  := oPrn:GetPrintableWidth()
    ...
    @ 0, 0 IMAGE oImg OF oPrn SIZE nW,nH FILENAME cFoto
   
  //  @ 0, 0 IMAGE oImg OF oPrn FILENAME cFoto
   
  // oimg:= Timage():New( 1, -100, 200, 200, oPrn, cFoto )
...
//   ? PRNINFOPAGLEFTMARGIN(oprn:hPrnInfo)
   
 //  ? PRNINFOPAGRIGHTMARGIN(oprn:hPrnInfo)
 //  ? PRNINFOPRIGHTMARGIN(oprn:hPrnInfo)
   
//   PRNINFOSETPRIGHTMARGIN(oprn:hPrnInfo, 50)
