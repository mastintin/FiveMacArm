...
function GenerateHbp( cPathProjecto )
   local cHbpContent := ""
   ...
   local cSdkPath  := GetPlistValue( cPrefFile, "PathSDK"  ) 
   ...
   cHbpContent += "# Flags Harbour" + CRLF
   ...
   cHbpContent += "# Paths" + CRLF

   cHbpContent += "-I" + cHarbourPath + "/include" + CRLF
   cHbpContent += "-I" + cFivePath + "/include" + CRLF

   cHbpContent += "-L" + cHarbourPath + "/lib" + CRLF
   cHbpContent += "-L" + cFivePath + "/lib" + CRLF
   ...
   cHbpContent += "-ldflag=-F" + cFivePath + "/frameworks" + CRLF
...
function BuildApp( cVersion, cIcon, oBtnRun )
   ...
   local cPath , cExeFile

   local cPrefFile := Path() + "/fivedit.plist"
   local oplist := TPlist():New( cPrefFile )
   local cHarbourPath := GetPlistValue( cPrefFile, "PathHarbour" )
   local cRoot :=  GetPlistValue( cPrefFile, "PathFiveMac" )
   local cHbmk2 := cHarbourPath + "/bin/hbmk2"
   ...
   if !file(cProjectName + ".hbp")
      msgAlert( " no encontrado el proyecto "+ cProjectName + ".hbp")
      return nil
   endif 
   if file(cProjectName)
      //msgInfo("borrando "+cProjectName)
      hb_FileDelete( cProjectName ) 
   endif
   cPath:= CurDir()
   ...
   if ! File( cExeFile )
      MsgAlert( "Binary not found: " + cExeFile )
      return nil
   endif

   CreaBuildStruct( cExeFile )
   ...
   WaitRun( "cp " + cExeFile + " " + cMacPath + "/" + cProjectName )
   ...
   CreateInfoFile( cFileNoPath(cProjectName), cIcon, cVersion )
   CreatePkInfo( cFileNoPath(cProjectName) )
...
Function CreaBuildStruct( cExeFile )

   local cFile := cFilenoPath(cExeFile)
   local cPath := cFilePath(cExeFile)

   local cAppPath, cResPath, cMacPath, cFwPath 
   

   if ! File( cFile )
      MsgAlert( " no se ha encontado el ejecutable " + cExeFile )
      Return nil
   endif

   cAppPath := cPath + cFile + ".app"
...
function RunApp()
   // Use cPathPrj if available (set in Loading or Gen), otherwise check relative
   local cApp
   
   if ! Empty( cPathPrj )
      cApp := cPathPrj + "/" + cProjectName + ".app"
   else
      cApp := hb_DirBase() + cProjectName + ".app"
   endif
