...
//oGet2:SetNumFormat("###0.##")    &&this does not work correct, if I enter something and then a dot everything disappears
   ...
   @ 45, 90 GET oGet2 VAR cText PICTURE "@!" OF oDlg TOOLTIP "a string"

     oGet2:bKeyDown := { |nkey| IF (nKey < 127 .AND. nKey <> 13, (RK_Search(oGet2:GetText, oget2:nPos), oGet2:SetFocus(), oGet2:SetSel(100,1)), ), IF (nKey == 13, oGet1:SetFocus(), ) }
...
function RK_Search(GetText, nSortOrder )
  local i
    local subtexto := substr( GetText,nSortOrder,1 )
    for i = 1 to 50
        Gettext += subtexto
    next
