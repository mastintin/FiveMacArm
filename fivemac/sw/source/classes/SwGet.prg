#include "FiveMac.ch"
 ...
 CLASS SwGet FROM TSwiftControl
    ...
    ASSIGN Value( u ) INLINE ::SetText( hb_ValToStr( u ) )
    ...
    METHOD OnValid()
    ASSIGN text( u ) INLINE ::OnAction( u )
 ...
 METHOD New( nTop, nLeft, nWidth, nHeight, uValue, oWnd, bAction, cPicture, bValid, lSecure, cPlaceholder ) CLASS SwGet
    ...
    // Usamos el motor estándar de la Isla
    ::Create( 14 )
 ...
 METHOD SetText( cText ) CLASS SwGet
    ::hState["text"] := cText
    SW_LOG( "🚢 [SwGet:SetText] enviando a Swift: " + hb_ValToStr( cText ) )
