...
HB_FUNC(CLRRED) { hb_retnll((HB_LONGLONG)[NSColor redColor]); }

HB_FUNC(CLRGREEN) { hb_retnll((HB_LONGLONG)[NSColor greenColor]); }

HB_FUNC(CLRBLUE) { hb_retnll((HB_LONGLONG)[NSColor blueColor]); }

HB_FUNC(COLOR_SET) { [((NSColor *)hb_parnll(1)) set]; }

HB_FUNC(PRNJOBCREATE) {
  NSPrintOperation *printjob;
  PrnView *view = (PrnView *)hb_parnll(1);
  printjob = [NSPrintOperation printOperationWithView:view];
  hb_retnll((HB_LONGLONG)printjob);
}

HB_FUNC(PRNVIEWCREATE) {
  PrnView *view =
      [[PrnView alloc] initWithFrame:NSMakeRect(hb_parnl(2), hb_parnl(1),
...
HB_FUNC(PRNINFOCREATE) {
  ...
  hb_retnll((HB_LONGLONG)pi);
}

HB_FUNC(PRNSETPAPERNAME) {
  NSString *string = hb_NSSTRING_par(2);
  NSPrintInfo *pi = (NSPrintInfo *)hb_parnll(1);
  NSPrinter *printer = [pi printer];

  if (printer == nil) {
    printer = [[NSPrintInfo sharedPrintInfo] printer];
  }

  NSSize paperSize = [printer pageSizeForPaper:string];
...
HB_FUNC(PRNINFOSHARED) {
  ...
  hb_retnll((HB_LONGLONG)sharedPrintInfo);
}

HB_FUNC(PRNINFOSCALE) {
  NSPrintInfo *pi = (NSPrintInfo *)hb_parnll(1);
  float scale =
      [[[pi dictionary] objectForKey:NSPrintScalingFactor] floatValue];
...
HB_FUNC(PRNINFOPRIGHTMARGIN) // in points
{
  NSPrintInfo *pi = (NSPrintInfo *)hb_parnll(1);
  hb_retnl([[[pi dictionary] objectForKey:NSPrintRightMargin] doubleValue]);
}

HB_FUNC(PRNINFOSETPRIGHTMARGIN) {
  NSPrintInfo *pi = (NSPrintInfo *)hb_parnll(1);

  [[pi dictionary] setObject:[NSNumber numberWithDouble:hb_parnl(2)]
...
HB_FUNC(PRNINFOPAGGETORIENTATION) {
  ...
  hb_retnll((HB_LONGLONG)nOrientation);
}
...
HB_FUNC(PRNSAY) // in points
{
  NSTextField *say =
      [[NSTextField alloc] initWithFrame:NSMakeRect(hb_parnl(2), hb_parnl(1),
                                                    hb_parnl(3), hb_parnl(4))];
  PrnView *window = (PrnView *)hb_parnll(5);
  ...
  [say setTextColor:(NSColor *)hb_parnll(9)];
  [say setBackgroundColor:(NSColor *)hb_parnll(10)];
  ...
  hb_retnll((HB_LONGLONG)say);
}

HB_FUNC(PRNSETSIZE) {
  PrnView *window = (PrnView *)hb_parnll(1);
