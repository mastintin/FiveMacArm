...
CGAutorelease(CGColorCreateGenericGray(0.0, 1.0));
    ...
    // AâÂ±adimos un contenedor donde almacenar nuestras CALayers de la
captura y su reflejo. CALayer *containerLayer = [CALayer layer];
    ...
    reflectionLayer.frame = CGRectOffset(screenLayer.frame,
                                                                         0.5,
                                                                         -NSHeight(displayBounds)
+ 0.5); reflectionLayer.transform = CATransform3DMakeScale(1.0, -1.0, 1.0); //
Volteamos el eje Y. reflectionLayer.sublayerTransform =
reflectionLayer.transform; [containerLayer addSublayer: reflectionLayer];

    // Creamos una CALayer para la sombra situada encima de la CALayer del
reflejo. CALayer *shadowLayer = [CALayer layer]; shadowLayer.frame =
reflectionLayer.bounds;
   // shadowLayer.delegate = self;
    // Ahara que somos el delegado de la CALayer, la lââ nea siguiente
invocarâÂ° el mâÂ©todo
    // -drawLayer:inContext: implementado mâÂ°s arriba.
    [shadowLayer setNeedsDisplay];
    [reflectionLayer addSublayer: shadowLayer];

    // AâÂ±adimos la CALayer contenedora a la CALayer del NSView de la
ventana principal. [contentView.layer addSublayer: containerLayer];
    ...
    // Creamos la animaciââ¥n con el efecto de encogimiento. Esto
harâÂ° que la imagen se vaya
    // haciendo mâÂ°s y mâÂ°s pequeâÂ±a.
    CABasicAnimation *shrinkAnimation = [CABasicAnimation animationWithKeyPath:
@"transform.scale"]; shrinkAnimation.timingFunction = [CAMediaTimingFunction
functionWithName: kCAMediaTimingFunctionEaseIn]; shrinkAnimation.toValue =
[NSNumber numberWithFloat: 0.0]; [containerLayer addAnimation: shrinkAnimation
forKey: @"shrinkAnimation"];

    // Creamos la animaciââ¥n con el efecto de desvanecimiento. Esto
harâÂ° que la imagen se vaya
    // desvaneciendo mientras se encoje.
    CABasicAnimation *fadeAnimation = [CABasicAnimation animationWithKeyPath:
@"opacity"]; fadeAnimation.toValue = [NSNumber numberWithFloat: 0.5];
    fadeAnimation.timingFunction = [CAMediaTimingFunction functionWithName:
kCAMediaTimingFunctionEaseIn]; [containerLayer addAnimation: fadeAnimation
forKey: @"fadeAnimation"];
   ...
   if ( salta )
   {
    ...
    // Creamos los key frame de la animaciââ¥n que darâÂ°n el
efecto de que la imagen da saltos
    // mientras se encoje y desvanece.
    CAKeyframeAnimation *positionAnimation = [CAKeyframeAnimation
animationWithKeyPath: @"position"]; CGMutablePathRef positionPath =
CGAutorelease(CGPathCreateMutable()); CGPathMoveToPoint(positionPath, NULL,
containerLayer.position.x, containerLayer.position.y);
...
kCAMediaTimingFunctionEaseIn]; [containerLayer addAnimation: positionAnimation
forKey: @"positionAnimation"];
    ...
    // Luego de hacer la captura, guardarla y realizar la animaciââ¥n
cerramos la aplicaciââ¥n.
   // [NSApp performSelector: @selector(terminate:) withObject: nil afterDelay:
DURATION_ANIMATION];
...
frame.size.width * vigourOfShake);
