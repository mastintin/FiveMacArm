...
- (void)drawRow:(NSInteger)row clipRect:(NSRect)clipRect {
  ...
  // NSColor * color = ( row % 2 ) ? [ NSColor lightGrayColor ] : [ NSColor
  // grayColor ]; [ color setFill ];
...
- (id)tableView:(Wbrowse *)aTableView
...
- (id)tableView:(Wbrowse *)aTableView
  ...
  if ([[[aTableColumn dataCell] className]
          isEqual:@"NSImageCell"]) // && hb_fsFile( hb_parc( -1 ) ) )
...
- (void)tableView:(Wbrowse *)aTableView
    willDisplayCell:(id)aCell
     forTableColumn:(NSTableColumn *)aTableColumn
                row:(NSInteger)rowIndex {
  // NSLog(@"siyo");
  // NSBeep();   // this beep sounds, so NSTableViewDataSource also calls this
  // willDisplayCell
}
...
HB_FUNC(BRWSETGRADICOLOR) {
  ...
  if (gradient && ![[browse selectedRowIndexes] containsIndex:row]) {
    ...
    NSRect gradientRect = NSMakeRect(
        3.0,
        ([browse rowHeight] + [browse intercellSpacing].height) * row + 1.0,
        rowRect.size.width - 4.0, rowRect.size.height - 2.0);
    NSBezierPath *smallerPath =
        [NSBezierPath bezierPathWithRoundedRect:gradientRect
                                        xRadius:9.0
                                        yRadius:9.0];
...
HB_FUNC(BRWSETCOLBMPTXT) // hTableView, nColumn
{
  ...
  BrwImageAndTextCell *imgTxtCell =
      [[[BrwImageAndTextCell alloc] init] autorelease];
...
- (NSRect)titleRectForBounds:(NSRect)cellRect {
  ...
  imageFrame.origin.y +=
      ceil((cellRect.size.height - imageFrame.size.height) / 2);
...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
  if (image != nil) {
    ...
    if ([self isGroupCell]) {
      // Center the text in the cellFrame, and call super to do thew ork of
      // actually drawing.
...
HB_FUNC( BRWSETCOLBTN ) // hTableView, nColumn
{
    ...
    NSTableColumn * column =  [ [ browse tableColumns ] objectAtIndex : (
hb_parnl( 2 ) - 1 ) ]; NSButtonCell * buttonCell = [ [ [ NSButtonCell alloc ]
init ] autorelease ];
...
HB_FUNC(BRWSETFONT) {
  ...
  NSFont *font = [[NSFont fontWithName:name size:hb_parnl(3)] autorelease];
...
HB_FUNC(BRWSETMENUFONT) {
  ...
  NSFont *font = [NSFont menuFontOfSize:(CGFloat)hb_parnl(2)];
  while (columnIndex--)
    [[(NSTableColumn *)[tableColumns objectAtIndex:columnIndex] dataCell]
        setFont:font];
...
HB_FUNC(BRWSETHEADHEIGHT) {
  ...
  if ([browse headerView]) {
    ...
    [[browse headerView] setFrame:frame];
  }
}
