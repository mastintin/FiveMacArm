...
HB_FUNC(WNDFROMNIB) {
  ...
  NSWindowController *wndController =
      [[NSWindowController alloc] initWithWindowNibName:string];
  ...
  while ([controls count] > 0) {
    ...
    // NSRunAlertPanel( [ control className ] ,@"yo", @"view", [ control
    // className ], NULL );
    ...
    // 	  {
    // 			  NSRunAlertPanel( @"outline" ,@"yo", @"view", NULL ,
    // NULL );
    //		 	 }
    //	 }
  }
...
HB_FUNC(WNDGETCONTROL) // hWnd, nId --> hCtrl
{
  ...
  hb_retnll((HB_LONGLONG)[GetView(window) viewWithTag:hb_parni(2)]);
}
...
HB_FUNC(WNDGETIDENTFROMNIB) {
  ...
  NSWindow *window = (NSWindow *)hb_parnll(1);
  NSArray *controls =
      [[[[window windowController] window] contentView] subviews];
  ...
  for (line = 0; line < [controls count]; line++) {
    ...
    if ([midenti isEqual:identif])
      return hb_retnll((HB_LONGLONG)control);
  }

  return hb_retnll((HB_LONGLONG)-1);
#endif
}
