...
HB_FUNC( CAPTURECAM )
{
   ...
   AVCaptureVideoPreviewLayer * vista = ( AVCaptureVideoPreviewLayer *)
hb_parnl( 1 );
...
AVMediaTypeVideo ];
   ...
   if(!device) {
    ...
    // AÒadimos la camara como dispositivo de entrada
    AVCaptureDeviceInput * mCaptureDeviceInput = [AVCaptureDeviceInput
...
HB_FUNC(CREATECAPDEVOUTPUT) {
  ...
  if ([[NSFileManager defaultManager] fileExistsAtPath:[destPath path]]) {
    ...
    if (![[NSFileManager defaultManager] removeItemAtPath:[destPath path]
                                                    error:&err]) {
      // MsgInfo(@"Error deleting existing movie %@",[err
...
HB_FUNC(AVOPENPANEL) {
  ...
  if ([openPanel runModal] == NSModalResponseOK) {
    AVPlayerItem *playerItem =
        [AVPlayerItem playerItemWithURL:[[openPanel URLs] objectAtIndex:0]];
    AVPlayer *videoPlayer = [AVPlayer playerWithPlayerItem:playerItem];
    [mview setPlayer:videoPlayer];
