...
@interface Mailer
    : NSObject <SBApplicationDelegate> //<NSSharingServiceDelegate>
{
...
@synthesize toField, fromField, subjectField, messageContent,
    fileAttachmentField;
...
- (IBAction)sendEmailMessage:(id)sender {
 ...
 sharingServiceNamed:NSSharingServiceNameComposeEmail]; service.delegate = self;
 ...
 //    NSAttributedString* textAttributedString = [self messageContent ] ;
    ...
    [ shareItems addObject : textAttributedString ] ;
     ...
     for(i=0; i<n; i++)
     {
         ...
         //  NSRunAlertPanel( attachmentFilePath ,@"yo", @"view",
 attachmentFilePath, NULL );
  ...
  MailOutgoingMessage *emailMessage = [[[mail
      classForScriptingClass:@"outgoing message"] alloc]
      initWithProperties:[NSDictionary
                             dictionaryWithObjectsAndKeys:[self subjectField],
                                                          @"subject",
                                                          [self messageContent],
                                                          @"content", nil]];
  ...
  MailToRecipient *theRecipient =
      [[[mail classForScriptingClass:@"to recipient"] alloc]
          initWithProperties:[NSDictionary
                                 dictionaryWithObjectsAndKeys:[self toField],
                                                              @"address", nil]];
  ...
  for (i = 0; i < n; i++) {
    ...
    // NSRunAlertPanel( attachmentFilePath ,@"yo", @"view",  attachmentFilePath,
    // NULL );
    ...
    if ([attachmentFilePath length] > 0) {
      ...
      theAttachment = [[[mail classForScriptingClass:@"attachment"] alloc]
          initWithProperties:@{
            @"fileName" : [NSURL fileURLWithPath:attachmentFilePath]
          }];

      //   theAttachment = [[[mail classForScriptingClass:@"attachment"] alloc]
      //   initWithProperties:
      //                    [NSDictionary dictionaryWithObjectsAndKeys:
      //                     [NSURL URLWithString:attachmentFilePath],
      //                     @"fileName", nil]];
  ...
  /*  NSString *attachmentFilePath = [self fileAttachmentField ];
     ...
     if ( [attachmentFilePath length] > 0 ) {
        ...
        theAttachment = [[[mail classForScriptingClass:@"attachment"] alloc]
                                          initWithProperties:
                                          [NSDictionary
    dictionaryWithObjectsAndKeys: attachmentFilePath, @"fileName", nil]];
