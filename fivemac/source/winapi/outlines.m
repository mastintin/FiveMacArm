...
@property(readwrite, retain) id bmp;
...
- (void)outlineView:(NSOutlineView *)outlineView
  ...
  if ([node.bmp isKindOfClass:[NSImage class]]) {
    image1 = (NSImage *)node.bmp;
  } else if ([node.bmp isKindOfClass:[NSString class]]) {
    NSFileManager *filemgr = [NSFileManager defaultManager];
    if ([filemgr fileExistsAtPath:(NSString *)node.bmp])
      image1 = [[[NSImage alloc] initWithContentsOfFile:(NSString *)node.bmp]
          autorelease];
    else
      image1 = ImgTemplate((NSString *)node.bmp);
  }
...
HB_FUNC(OUTLINECREATE) {
  ...
  MOutlineView *outlineView =
      [[MOutlineView alloc] initWithFrame:[[sv contentView] frame]];
  ...
  [[[outlineView enclosingScrollView] contentView]
