...
HB_FUNC(TXTCREATE) // hWnd
{
  ...
  memo = [[TextView alloc] initWithFrame:[[sv contentView] frame]];
  [memo setDelegate:memo];

  [sv setDocumentView:memo];
  [GetView(window) addSubview:sv];

  hb_retnll((HB_LONGLONG)memo);
}
...
HB_FUNC(TXTGETRTF) {
  ...
  hb_retc([[[NSString alloc] initWithData:rtfData encoding:NSUTF8StringEncoding]
...
HB_FUNC(TXTCOL) {
  ...
  unsigned int uiAt = [memo selectedRange].location;
  unsigned int uiEnd =
      [[memo string] lineRangeForRange:[memo selectedRange]].location - 1;
...
HB_FUNC(TXTSETFONT) {
  ...
  [memo setFont:[[NSFont fontWithName:name size:hb_parnl(3)] autorelease]];
}

HB_FUNC(TXTHIGHLIGHT) {
  TextView *memo = (TextView *)hb_parnll(1);

  [[memo textStorage] addAttribute:NSForegroundColorAttributeName
...
HB_FUNC(TXTADDRULERVERT) {
  ...
  NSRulerView *rulerV =
      [[NSRulerView alloc] initWithScrollView:sv orientation:NSVerticalRuler];
...
HB_FUNC(TXTADDRULERHORI) {
  ...
  NSRulerView *rulerH =
      [[NSRulerView alloc] initWithScrollView:sv orientation:NSHorizontalRuler];
