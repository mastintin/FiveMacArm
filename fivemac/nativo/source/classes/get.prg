...
//#define HB_LANG_ITEM_BASE_TEXT  1000
...
CLASS TGet FROM TControl
    ...
    METHOD New( nTop, nLeft, nWidth, nHeight, oWnd, bSetGet, bValid, lUpdate,;
        ...
        cVarName, cPicture, cCueText, lUtf, bWhen )
...
METHOD New( nTop, nLeft, nWidth, nHeight, oWnd, bSetGet, bValid, lUpdate,;
        ...
        cVarName, cPicture, cCueText, lUtf, bWhen ) CLASS TGet
...
METHOD Refresh() CLASS TGet
    ...
    if ::bSetGet != nil
        uVal := Eval( ::bSetGet )
        if ValType( uVal ) == "N" .or. ValType( uVal ) == "D"
            uVal := Transform( uVal, ::cPicture )
        endif
        ::SetText( cValToChar( uVal ) )
    endif
...
Static Function NumValidate( cText )
    ...
    for nForIdx := 1 TO Len( cText )
        if hb_asciiUpper( SubStr( cText, nForIdx, 1 ) ) $  "1234567890."
            lTrue := .t.
        else
            lTrue := .f.
            exit
