...
- (void)drawWithFrame:(NSRect)cellFrame inView:(NSView *)controlView {
  if (image != nil) {
    ...
    newFrame.origin.x -= 25; newFrame.size.width += 25;
...
- (void)tableView:(Wbrowse *)tableView mouseDownInHeaderOfTableColumn:(NSTableColumn *)aTableColumn;
...
- (void)tableView:(NSTableView *)aTableView setObjectValue:(id)aData forTableColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex;
...
- (void)BrwDblClick:(id)sender {
  if (symFMH == NULL) symFMH = hb_dynsymSymbol(hb_dynsymFindName("_FMH"));
...
- (id)tableView:(NSTableView *)aTableView objectValueForTableColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex {
  ...
  if ([[cell className] isEqualToString:@"NSImageCell"]) {
    [pool release];
    return ([string length] > 0) ? [[[NSImage alloc] initWithContentsOfFile:string] autorelease] : nil;
  }
  
  if ([[cell className] isEqualToString:@"BrwImageAndTextCell"]) {
     NSString *filename = [NSString stringWithUTF8String:hb_parvc(-1, 1)];
     NSImage *image = (hb_parvl(-1, 3)) ? [[NSWorkspace sharedWorkspace] iconForFile:filename] : [[[NSImage alloc] initWithContentsOfFile:filename] autorelease];
...
- (void)keyDown:(NSEvent *)theEvent {
  ...
  hb_vmDo(4);
  if (hb_parnl(-1) != 1) [super keyDown:theEvent];
}
...
HB_FUNC(BRWCREATE) {
  NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
  NSScrollView *sv = [[NSScrollView alloc] initWithFrame:NSMakeRect(hb_parnl(2), hb_parnl(1), hb_parnl(3), hb_parnl(4))];
  Wbrowse *browse;
  ...
  browse->nRow = 0; browse->nCol = 0;
  [browse release]; [sv release];
...
HB_FUNC(BRWSETSIZE) {
  ...
  [[browse enclosingScrollView] setFrameSize:NSMakeSize(hb_parnl(2), hb_parnl(3))];
}

HB_FUNC(BRWREFRESH) {
  [(Wbrowse *)hb_parnll(1) reloadData];
}

HB_FUNC(BRWADDCOLUMN) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  NSTableColumn *column = [[NSTableColumn alloc] init];
  [column setIdentifier:[NSString stringWithFormat:@"%i", (int)[browse numberOfColumns]]];
  [column setWidth:100]; [column setEditable:NO];
...
HB_FUNC(COLSETHEADER) {
  [[(NSTableColumn *)hb_parnll(1) headerCell] setStringValue:hb_NSSTRING_par(2)];
}

HB_FUNC(BRWGOTOP) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  [browse selectRowIndexes:[NSIndexSet indexSetWithIndex:0] byExtendingSelection:NO];
...
HB_FUNC(BRWGOBOTTOM) {
  ...
  if (iRows > 0) {
    [browse selectRowIndexes:[NSIndexSet indexSetWithIndex:iRows - 1] byExtendingSelection:NO];
...
HB_FUNC(BRWGODOWN) {
  ...
  if (row < [browse numberOfRows] - 1) {
    [browse selectRowIndexes:[NSIndexSet indexSetWithIndex:row+1] byExtendingSelection:NO];
...
HB_FUNC(BRWROWPOS) {
  hb_retnl([(Wbrowse *)hb_parnll(1) selectedRow] + 1);
}

HB_FUNC(BRWSETROWPOS) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  NSInteger row = hb_parnl(2) - 1;
  [browse selectRowIndexes:[NSIndexSet indexSetWithIndex:row] byExtendingSelection:NO];
  [browse scrollRowToVisible:row];
}

HB_FUNC(BRWSETSELECT) {
  [(Wbrowse *)hb_parnll(1) selectRowIndexes:[NSIndexSet indexSetWithIndex:hb_parnl(2) - 1] byExtendingSelection:NO];
}
...
HB_FUNC(BRWGETGRIDLINES) {
  hb_retnl([(Wbrowse *)hb_parnll(1) gridStyleMask]);
}
...
HB_FUNC(BRWSETTEXTCOLOR) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  for (NSTableColumn *col in [browse tableColumns]) [[col dataCell] setTextColor:(NSColor *)hb_parnll(2)];
}

HB_FUNC(BRWSETFONT) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  NSFont *font = [NSFont fontWithName:hb_NSSTRING_par(2) size:hb_parnl(3)];
  for (NSTableColumn *col in [browse tableColumns]) [[col dataCell] setFont:font];
}

HB_FUNC(BRWSETROWHEIGHT) {
  [(Wbrowse *)hb_parnll(1) setRowHeight:hb_parnd(2)];
}

HB_FUNC(BRWGETROWHEIGHT) {
  hb_retnd([(Wbrowse *)hb_parnll(1) rowHeight]);
}
...
HB_FUNC(BRWGETCOLWIDTH) {
  hb_retnd([[[(Wbrowse *)hb_parnll(1) tableColumns] objectAtIndex:hb_parnl(2)-1] width]);
}
...
HB_FUNC(BRWCLRDBLACTION) {
  [(Wbrowse *)hb_parnll(1) setDoubleAction:nil];
}

HB_FUNC(BRWCOLPOS) {
  hb_retnl([(Wbrowse *)hb_parnll(1) clickedColumn] + 1);
}
...
HB_FUNC(BRWSCROLLSTYLE) {
  ...
  if ([sv respondsToSelector:@selector(setScrollerStyle:)]) [sv setScrollerStyle:hb_parnl(2)];
}

HB_FUNC(BRWSCROLLVSHOW) {
  [[(Wbrowse *)hb_parnll(1) enclosingScrollView] setHasVerticalScroller:hb_parl(2)];
}

HB_FUNC(BRWSCROLLHSHOW) {
  [[(Wbrowse *)hb_parnll(1) enclosingScrollView] setHasHorizontalScroller:hb_parl(2)];
}

HB_FUNC(BRWSCROLLAUTOHIDE) {
  [[(Wbrowse *)hb_parnll(1) enclosingScrollView] setAutohidesScrollers:hb_parl(2)];
}

HB_FUNC(BRWEDIT) {
  [(Wbrowse *)hb_parnll(1) editColumn:hb_parnl(2)-1 row:hb_parnl(3)-1 withEvent:nil select:YES];
}

HB_FUNC(BRWSETNOHEAD) {
  if (hb_parl(2)) [(Wbrowse *)hb_parnll(1) setHeaderView:nil];
}

HB_FUNC(BRWSETHEADHEIGHT) {}

HB_FUNC(BRWAUTOAJUST) {
  [(Wbrowse *)hb_parnll(1) sizeToFit];
}

HB_FUNC(BRWSETSYSTEMFONT) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  for (NSTableColumn *col in [browse tableColumns]) [[col dataCell] setFont:[NSFont systemFontOfSize:hb_parnl(2)]];
}

HB_FUNC(BRWSETMENUFONT) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  for (NSTableColumn *col in [browse tableColumns]) [[col dataCell] setFont:[NSFont menuFontOfSize:hb_parnl(2)]];
}
...
HB_FUNC(BRWSETINDICATORASCEND) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  [browse setIndicatorImage:[NSImage imageNamed:@"NSAscendingSortIndicator"] inTableColumn:[[browse tableColumns] objectAtIndex:hb_parnl(2)-1]];
}

HB_FUNC(BRWSETINDICATORDESCENT) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  [browse setIndicatorImage:[NSImage imageNamed:@"NSDescendingSortIndicator"] inTableColumn:[[browse tableColumns] objectAtIndex:hb_parnl(2)-1]];
}

HB_FUNC(BRWSETNOINDICATOR) {
  Wbrowse *browse = (Wbrowse *)hb_parnll(1);
  [browse setIndicatorImage:nil inTableColumn:[[browse tableColumns] objectAtIndex:hb_parnl(2)-1]];
}
